...
import pageObject.liveGuru.user.AbstractLiveGuruPageUIs;
...
public abstract class AbstractPage {
	...
	jsExecutor = (JavascriptExecutor) driver;
	boolean status = (boolean) jsExecutor.executeScript("return  arguments[0].complete && typeof arguments[0].naturalWidth != \"undefined\" && arguments[0].naturalWidth > 0", find(driver, castToObject(xpathValue, values)));
	if (status) {
	    return true;
	} else {
	    return false;
...
//	}
	...
	return isElementDisplayed(driver, AbstractWordPressPageUIs.DYNAMIC_IMAGE_AVATAR_POST_AT_TITLE, titleValue, imageValue) && isImageLoaded(driver, AbstractWordPressPageUIs.DYNAMIC_IMAGE_AVATAR_POST_AT_TITLE, titleValue, imageValue);
    }
	...
	waitElementClickable(driver, AbstractLiveGuruPageUIs.DYNAMIC_BUTTON_BY_TITLE, titleValue);
	clickToElement(driver, AbstractLiveGuruPageUIs.DYNAMIC_BUTTON_BY_TITLE, titleValue);
    }
    /* Dynamic Nopcommerce page */
